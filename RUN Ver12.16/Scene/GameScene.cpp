...
<<<<<<< HEAD
	...
	{
		lpSceneMng.AddDrawQue({ IMAGE_ID("ｴﾌｪｸﾄ")[0],lpSceneMng.GameSize.x / 2.0,lpSceneMng.GameSize.y / 2.0,0.0,0, LAYER::CHAR });
	}

=======
	if (lpSceneMng.bgSpeed >= 14)
	{
		lpSceneMng.AddDrawQue({ IMAGE_ID("ｹﾞｰﾑｴﾌｪｸﾄ")[(SceneCount / 5) % 4],UICenter.x,lpSceneMng.GameSize.y / 2,0.0,INT_MAX,LAYER::SYSTEM });
	}
